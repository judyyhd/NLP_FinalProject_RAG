...
#SBATCH --gres=gpu:2
#SBATCH --output=/home/hy1331/nlp_final/logs/no_rag_%j.log
#SBATCH --error=/home/hy1331/nlp_final/logs/no_rag_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=hy1331@nyu.edu

# No RAG Evaluation on HotpotQA Data
# This script runs baseline evaluation WITHOUT retrieval to test the model's general performance
# Usage: sbatch run_no_rag.sh [DATA_TYPE] [DATA_SIZE]
#   DATA_TYPE: dev, train, or test (default: dev)
#   DATA_SIZE: 10, 100, 500, 1000, 5000, 10000, or full (default: 10)
# Examples:
#   sbatch run_no_rag.sh dev 10
#   sbatch run_no_rag.sh dev 500
#   sbatch run_no_rag.sh train 1000
#   sbatch run_no_rag.sh test full
...
METRIC="em"
MODE="no_rag"  # NO RETRIEVAL - just direct model inference
...
run_evaluation() {
    ...
    print_status "Running evaluation on ${subset_name} subset (NO RAG)..."
...
print_header "NO RAG Evaluation on HotpotQA ${DATA_TYPE} Data (${DATA_SIZE} examples)"

# Create descriptive symlinks to log files
if [ -n "${SLURM_JOB_ID}" ]; then
    LOG_DIR="${PROJECT_DIR}/logs"
    ln -sf "no_rag_${SLURM_JOB_ID}.log" "${LOG_DIR}/no_rag_${DATA_TYPE}${DATA_SIZE}_latest.log"
    ln -sf "no_rag_${SLURM_JOB_ID}.err" "${LOG_DIR}/no_rag_${DATA_TYPE}${DATA_SIZE}_latest.err"
    print_status "Log files: no_rag_${DATA_TYPE}${DATA_SIZE}_latest.{log,err}"
fi
...
print_status "Mode: ${MODE} (NO RETRIEVAL)"
...
print_header "Running Evaluations (Without Retrieval)"
...
for dataset_info in "${DATASETS[@]}"; do
    ...
    output_file="${RESULTS_DIR}/no_rag_${DATA_TYPE}${DATA_SIZE}_results.json"
...
echo ""
ls -lh "${RESULTS_DIR}"/no_rag_*_results.json | awk '{print "  " $9 " (" $5 ")"}'

print_status "To compare with vanilla RAG results:"
echo ""
echo "  python3 -c \""
echo "import json; import numpy as np"
echo ""
echo "def load_results(filename):"
echo "    data = []"
...
echo "    return data"
echo ""
echo "# Load both sets of results"
echo "no_rag = load_results('${RESULTS_DIR}/no_rag_dev500_results.json')"
echo "vanilla_rag = load_results('${RESULTS_DIR}/vanilla_rag_dev500_results.json')"
echo ""
echo "# Calculate metrics"
echo "for name, data in [('No RAG', no_rag), ('Vanilla RAG', vanilla_rag)]:"
echo "    em = np.mean([item.get('em_score', 0) for item in data])"
echo "    f1 = np.mean([item.get('f1_score', 0) for item in data])"
echo "    print(f'{name:12s}: EM={em:.4f}, F1={f1:.4f}')"
echo "\""

# Print summary table of all results
echo ""
print_header "FINAL RESULTS SUMMARY (NO RAG)"
echo ""
...
print_status "This baseline shows model performance WITHOUT any retrieval augmentation"
