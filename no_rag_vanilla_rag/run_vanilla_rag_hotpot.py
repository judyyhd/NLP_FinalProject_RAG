...
from metrics import metric_max_over_ground_truths, exact_match_score, match, qa_f1_score
import ast
import backoff
import openai
...
ORG_KEY = os.getenv("OPENAI_ORG_KEY", "YOUR_ORG_KEY")
...
def main():
    ...
    parser.add_argument('--mode', type=str, default="no_rag",
                        help="Mode: 'retrieval' for vanilla RAG, 'no_rag' or 'vanilla' for direct inference without retrieval")
    ...
    for item in input_data:
        if "golds" not in item:
            if "answers" in item:
                ...
                # Handle singular 'answer' field
                item["golds"] = [item["answer"]] if isinstance(item["answer"], str) else item["answer"]
                ...
                # Fallback: empty list to prevent crashes
                print(f"Warning: No answer field found for item {item.get('id', 'unknown')}")
                item["golds"] = []
    ...
    for item in input_data:
        # Ensure golds field exists (defensive check)
        if "golds" not in item or not item["golds"]:
            print(f"Warning: Missing or empty 'golds' for item {item.get('id', 'unknown')}, skipping metrics")
            item["em_score"] = 0.0
            item["f1_score"] = 0.0
            item["metric_result"] = 0.0
            continue
            
        # Compute Exact Match (EM)
        em_result = metric_max_over_ground_truths(
            exact_match_score, item["output"], item["golds"])
        
        # Compute F1 Score
        f1_result = metric_max_over_ground_truths(
            qa_f1_score, item["output"], item["golds"])
        
        # Compute other metrics based on specified metric
        if args.metric == "em":
            metric_result = em_result
            ...
            metric_result = f1_result
        else:
            # Default to EM if no metric specified
            metric_result = em_result
        
        # Store all metrics in the item
        item["em_score"] = em_result
        item["f1_score"] = f1_result
        item["metric_result"] = metric_result

    # Calculate and print summary statistics
    em_scores = [item["em_score"] for item in input_data]
    f1_scores = [item["f1_score"] for item in input_data]
    primary_scores = [item["metric_result"] for item in input_data]
    
    print("=" * 60)
    print("EVALUATION RESULTS SUMMARY")
    print("=" * 60)
    print(f"Total examples evaluated: {len(input_data)}")
    print(f"Exact Match (EM) Score: {np.mean(em_scores):.4f}")
    print(f"F1 Score: {np.mean(f1_scores):.4f}")
    print(f"Primary metric ({args.metric if args.metric else 'em'}): {np.mean(primary_scores):.4f}")
    print("=" * 60)
