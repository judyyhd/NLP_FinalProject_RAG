    ...
    if mode == "always_retrieve":
        ...
        if threshold is not None:
            ...
            for tok, id in ret_tokens.items():
                if len(pred_log_probs) == 0 or id not in pred_log_probs[0]:
                    score_dict[tok] = -100
                    continue
                # prob = pred_log_probs[0][id].logprob
                # NEW (Ghina)
                logprob_val = pred_log_probs[0][id]
                prob = logprob_val.logprob if hasattr(logprob_val, 'logprob') else logprob_val
    ...
    if do_retrieve is True:
        ...
        for p_idx, pred in enumerate(preds):
            ...
            for tok, id in rel_tokens.items():
                ...
                # NEW (Ghina): Handle both old and new vLLM API
                if len(pred_log_probs) > 0 and id in pred_log_probs[0]:
                    logprob_val = pred_log_probs[0][id]
                    prob = logprob_val.logprob if hasattr(logprob_val, 'logprob') else logprob_val
                else:
                    prob = -100
                ...
                if len(groundness_token_appear_indices) > 0:
                    ...
                    for token, token_id in grd_tokens.items():
                        ...
                        if token_id in pred_log_probs[idx]:
                            logprob_val = pred_log_probs[idx][token_id]
                            prob = logprob_val.logprob if hasattr(logprob_val, 'logprob') else logprob_val
                        else:
                            prob = -100
            ...
            results["retrieval_{}".format(p_idx)] = {
                ...
                "ground_score": ground_score,
                "utility_score": utility_score,
                "relevance_score_dict": relevance_score_dict[p_idx],
                "grd_score_dict": grd_score_dict[p_idx],
                "ut_score_dict": ut_score_dict[p_idx]}
...
def main():
    ...
    for i, row in tqdm(enumerate(input_data)):
        ...
        # Skip if prompt + evidence would be too long
        prompt_tokens = len(tokenizer.encode(prompt))
        # Estimate max evidence length (worst case: 10 docs * ~150 tokens each)
        if prompt_tokens > 1200:  # Leave room for evidence and generation
            print(f"Skipping example {i}: prompt too long ({prompt_tokens} tokens)")
            preds.append("SKIPPED")
        ...
        try:
            ...
            if "longer than the maximum model length" in str(e):
                print(f"Skipping example {i}: context too long")
                preds.append("SKIPPED")
                prompts.append(prompt)
                all_results.append({})
                metric_results.append(0)
